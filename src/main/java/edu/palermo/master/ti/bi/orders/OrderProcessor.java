...
public class OrderProcessor implements ItemProcessor<OrderRecord, Order> {
	...
	public Order process(OrderRecord item) {
		final Order order = Order.builder()
				...
				.createdDate(formatDate(item.getCreatedDate()))
				.updatedDate(formatDate(item.getUpdatedDate()))
				.submittedDate(formatDate(item.getSubmittedDate()))
				.deliveryDate(formatDate(item.getDeliveryDate()))
